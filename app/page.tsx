...
export default function Home() {
  ...
  // poster lookup for history strip
  const allMovies = [...recommended, ...searchResults]
  const posterById = (id: number): string =>
    allMovies.find(m => m.movieId === id)?.poster ?? ''
            ...
            {watchHistory.length > 0 && (
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                ...
                {watchHistory.slice(-5).reverse().map((id, i) => (
                  <div key={id} style={{
                    width: 32, height: 44, borderRadius: 6, overflow: 'hidden',
                    border: '1px solid rgba(99,102,241,0.27)',
                    opacity: 1 - i * 0.15,
                    background: '#1a1a3a', flexShrink: 0,
                  }}>
                    <img
                      src={posterById(id)}
                      alt=""
